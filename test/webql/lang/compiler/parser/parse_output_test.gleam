...
import webql/lang/compiler/parser/parse_output
import webql/lang/compiler/source

pub fn parse_node_port_output_test() {
  let source = "m.out"
  ...
  assert output
    == ast.PortOutput(span: source.Span(start: 0, end: 5), path: ["m", "out"])

  assert rest
    == [token.Token(kind: token.EOF, span: source.Span(start: 5, end: 5))]
}

pub fn parse_operation_port_output_test() {
  let source = ".out"
  ...
  assert output
    == ast.PortOutput(span: source.Span(start: 0, end: 4), path: ["out"])

  assert rest
    == [token.Token(kind: token.EOF, span: source.Span(start: 4, end: 4))]
}

pub fn parse_primitive_output_test() {
  let source = "123"
...
pub fn parse_returns_error_when_space_exists_between_node_alias_and_dot_test() {
  ...
  let assert Error(error) = parse_output.parse(source, tokens)
