...
pub fn lex_upper_identifier_stops_correctly_test() {
  ...
  assert tok
    == token.Token(kind: token.UpperIdentifier, span: source.Span(start: 0, end: 7))
  assert rest == <<"(":utf8>>
}
...
pub fn lex_upper_identifier_allows_letters_and_digits_test() {
  ...
  assert tok
    == token.Token(kind: token.UpperIdentifier, span: source.Span(start: 4, end: 9))
  assert rest == <<",":utf8>>
}
...
pub fn lex_upper_identifier_stops_before_underscore_test() {
  ...
  assert tok
    == token.Token(kind: token.UpperIdentifier, span: source.Span(start: 0, end: 4))
