...
pub fn run_plan_routes_parameter_to_output_test() {
  ...
  let assert Ok(returns) =
    run_plan.run(p, dict.from_list([#("input", dynamic.int(7))]))

  let assert Ok(value) = dict.get(returns, "output")
  assert decode.run(value, decode.int) == Ok(7)
}

pub fn run_plan_routes_constant_to_output_test() {
  let p =
    plan.Plan(
  ...
  let assert Ok(returns) = run_plan.run(p, dict.new())

  let assert Ok(value) = dict.get(returns, "output")
  assert decode.run(value, decode.int) == Ok(99)
}

pub fn run_plan_runs_step_and_returns_output_test() {
  let step =
    plan.Step(
      name: "inc",
  ...
  let assert Ok(returns) =
    run_plan.run(p, dict.from_list([#("input", dynamic.int(4))]))

  let assert Ok(value) = dict.get(returns, "output")
  assert decode.run(value, decode.int) == Ok(5)
}

pub fn run_plan_runs_multiple_batches_in_sequence_test() {
  let step_a =
    plan.Step(
      name: "double",
      resolver: plan.FunctionResolver(
  ...
  let assert Ok(returns) =
    run_plan.run(p, dict.from_list([#("input", dynamic.int(3))]))
