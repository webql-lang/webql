-module(webql@interpreter@progress).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch, inline]).
-define(FILEPATH, "src/webql/interpreter/progress.gleam").
...
-file("src/webql/interpreter/progress.gleam", 134).
...
decode(Unknown) ->
    ...
    case gleam@dynamic@decode:run(Unknown, Schema) of
        {ok, Result} ->
            {ok, Result};
...
-file("src/webql/interpreter/progress.gleam", 11).
?DOC(" Stores initial plan parameters as root-level values.\n").
-spec add_parameters(webql@memory:memory(GIR), gleam@dynamic:dynamic_()) -> {ok,
        webql@memory:memory(GIR)} |
...
-file("src/webql/interpreter/progress.gleam", 31).
?DOC(" Stores outputs produced by a completed step.\n").
-spec add_outputs(webql@memory:memory(GIW), binary(), gleam@dynamic:dynamic_()) -> {ok,
        webql@memory:memory(GIW)} |
    {error, webql@interpreter@diagnostic:diagnostic()}.
...
-file("src/webql/interpreter/progress.gleam", 124).
...
-file("src/webql/interpreter/progress.gleam", 54).
...
-file("src/webql/interpreter/progress.gleam", 94).
