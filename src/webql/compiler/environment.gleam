...
/// Creates a new environment from a loaded schema.
pub fn new(schema: schema.Schema) -> Environment {
  let schema.Schema(inputs:, nodes:, outputs:, typenames:) = schema
  Environment(inputs:, nodes:, outputs:, typenames:)
}
